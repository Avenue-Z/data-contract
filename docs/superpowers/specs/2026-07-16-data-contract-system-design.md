...
2. **Distribution + no stable public API → R9.** Editable-local-path install and deep imports are an
   adoption blocker, invisible from inside the pilot. *Designed 2026-07-20, implemented 2026-07-21;
   **not closed until `v0.1.0` is tagged on `main`** — until the tag exists a consumer pinning it
   gets a resolve failure, so the code alone delivers nothing. §15 item 7 (the import-time-crash
   concern) is closed by the same work: `ContractRuntime.disabled()` and `load_runtime(enabled=...)`
   give the lazy/opt-out path it asked for.*
