...
| R10 | **The authored format is itself unversioned.** The schema/contract YAML is the real API surface but carries no format-version; a `contract-core` format change breaks every repo's authored files with no migration — the system has a compatibility story for *data* (§9) but none for *its own artifacts*. | Format evolution across repos | **Phase 1/2** | Add an explicit format `apiVersion` to the schema and contract formats before broad adoption; define a format-compat policy (loudly reject/migrate an unknown format version, never silently mis-parse). **CLOSED 2026-07-22** (`613d011`–`55980df`), released as `v0.3.0`. Took the declarative-key option, not a bare `apiVersion` stamp: authored files are now **strict** — `extra="forbid"` on `Schema`/`Field`/`Contract`/`BoundarySpec` refuses a key `contract-core` does not recognise at load rather than silently dropping it — and carry an optional `format_version` (`v1`; a missing stamp reads as `v1`), refused with an upgrade hint if this reader cannot carry it forward. `ContractFormatError` joins the public surface (5 → 6), raised only at the `from_yaml` boundary and carrying `.path`/`.errors`/`.hint`. See `CHANGELOG.md`'s `[0.3.0]` entry for the full BREAKING note and the minimum-supported-reader statement. **Residual, not sealed:** nothing here reaches a reader already in the wild — `0.1.0` and `0.2.0` neither forbid extras nor check `format_version`, so a `0.1.0` runtime would go on silently dropping `0.2.0`'s constraint keys forever if it were still pinned anywhere. It isn't (consuming repos now pin `>= 0.3.0`), which is what makes that reader *unreachable*, not fixed. R10 buys the *next* format break, not the one already behind us — the `0.1.0`→`0.2.0` gap is not sealed by this work (R10 design §6). |
...
2. **Distribution + no stable public API → R9.** Editable-local-path install and deep imports are an
   ...
   for.*
3. **The authored format is unversioned → R10.** ***CLOSED 2026-07-22** — see the R10 row in
   §14 for what shipped (strict keys, `format_version`, `ContractFormatError`) and the stated
   residual. Released as `v0.3.0`; see `CHANGELOG.md`'s `[0.3.0]` entry for the release notes.*
...
5. **Library-hygiene defects to clear alongside the above:** ~~`ContractRuntime.REGISTRY` is
   ...
   already order-dependent)~~ — **CLOSED 2026-07-23** (`be573ec`, PR #29). `REGISTRY` now records
   registrations keyed by `(system, direction, name)` instead of by contract alone, `_reset_registry()`
   gives an explicit clear, and an autouse pytest fixture calls it before every test — registry leakage
   across tests is closed structurally rather than left order-dependent. Internal behavior change only;
   no public-API impact. **Only the registry slice is closed** — the remaining entries below (error-
   classification fragility, the `schema`-field shadowing, and the event log's missing sink/reader)
   are unaddressed by this work and stay explicitly open. ~~an empty (0-row) result frame is reported as "all columns missing" instead
...
clearing the gate was a precondition for adoption, not the phase. `reconcile` and its R3
registration-completeness work, and R2's drift-test enforcement, are **now closed** (§14, R3/R2,
2026-07-23). Still open before "new automations get contracts by default" is true: the §5.5 authoring
skill and R7's adoption measurement. Item 5's library-hygiene list is also still open, less the
registry entry closed alongside R3/R2 above — one remaining entry now matters more than it did:
**the event log still has no reader.** `v0.2.0`'s documented adoption path is "adopt the
